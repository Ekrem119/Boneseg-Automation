...
class PalettisedPNGSegmentLoader:
    def __init__(self, video_png_root):
        ...
        for filename in png_filenames:
            ...
            try:
                # garder un identifiant numérique si possible
                frame_id_int = int(frame_id)
                self.frame_id_to_png_filename[frame_id_int] = filename
            except ValueError:
    ...
    def load(self, frame_id):
        ...
        if frame_id not in self.frame_id_to_png_filename:
            raise KeyError(f"Frame id {frame_id} not found in {self.video_png_root}")
        # check the path
...
class MultiplePNGSegmentLoader:
    ...
    def _load_multiple_pngs(self, frame_id):
        ...
        for obj_folder in all_objects:
            obj_id_str = os.path.basename(obj_folder)
            try:
                obj_id = int(obj_id_str) + 1  # offset 1 as bg is 0
            except ValueError:
                obj_id = obj_id_str  # garder la chaîne si ce n’est pas un entier
