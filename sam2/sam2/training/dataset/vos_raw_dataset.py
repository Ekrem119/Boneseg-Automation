...
class PNGRawDataset(VOSRawDataset):
    ...
    def get_video(self, idx):
        ...
        img_dir = os.path.join(self.img_folder, name)
        mask_dir = os.path.join(self.gt_folder, name)

        # Choix du loader pour les masques
        if self.is_palette:
            segment_loader = PalettisedPNGSegmentLoader(mask_dir)
        else:
            segment_loader = MultiplePNGSegmentLoader(mask_dir, single_object_mode=False)

        # Récupérer toutes les images .png dans le dossier
        all_frames = sorted(glob.glob(os.path.join(img_dir, "*.png")))

        # Construire les frames (une seule image par dossier dans ton cas)
        frames = [
            VOSFrame(os.path.splitext(os.path.basename(fpath))[0], image_path=fpath)
        ...
        # Créer l'objet vidéo
        video = VOSVideo(name, idx, frames)
...
class SA1BRawDataset(VOSRawDataset):
    ...
    def get_video(self, idx):
        ...
        # Utiliser idx comme identifiant numérique pour video_id
...
class JSONRawDataset(VOSRawDataset):
    ...
    def get_video(self, video_idx):
        ...
        all_frame_names = sorted(os.listdir(os.path.join(self.img_folder, video_name)))

        frames = [
            VOSFrame(
                i,   # identifiant numérique
                image_path=os.path.join(self.img_folder, video_name, frame_name)
            )
