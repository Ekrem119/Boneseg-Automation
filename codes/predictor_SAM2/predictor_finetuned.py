...
# --- Config et checkpoint ---
model_cfg = r"C:\Users\Bicel service\Desktop\USER\Maria\sam2\configs\sam2.1\sam2.1_hiera_b+.yaml"
#finetuned_ckpt = r"C:\Users\Bicel service\Desktop\USER\Maria\datasets\finetuned_checkpoints\checkpoints\checkpoint_blanc_DAPI.pt"
#finetuned_ckpt = r"C:\Users\Bicel service\Desktop\USER\Maria\datasets\finetuned_checkpoints\checkpoints\checkpoint_blanc_BrightField.pt"
#finetuned_ckpt = r"C:\Users\Bicel service\Desktop\USER\Maria\datasets\finetuned_checkpoints\checkpoints\checkpoint_alteration.pt"
#finetuned_ckpt = r"C:\Users\Bicel service\Desktop\USER\Maria\datasets\finetuned_checkpoints\checkpoints\checkpoint_canaux_normaux.pt"
finetuned_ckpt = r"C:\Users\Bicel service\Desktop\USER\Maria\datasets\finetuned_checkpoints\checkpoints\checkpoint_objet.pt"
...
def compute_objects_metrics(mask, min_area=50):
    """
    Calcule les métriques pour chaque objet distinct dans un masque binaire.
    """
    rows = []
    lab = label(mask)
...
def run_predict(x, y):
    ...
    # --- calcul des métriques pour les objets distincts du meilleur masque ---
    df_stats = compute_objects_metrics(best_mask, min_area=50)
    num_objects = len(df_stats)
    total_area = float(df_stats["area_px"].sum()) if num_objects else 0.0

    # --- Intensité intérieur vs extérieur ---
    img32 = np.array(image, dtype=np.float32)
    inside_vals = img32[best_mask > 0]
    outside_vals = img32[best_mask == 0]

    mean_inside = inside_vals.mean() if inside_vals.size > 0 else np.nan
    mean_outside = outside_vals.mean() if outside_vals.size > 0 else np.nan
    ratio = mean_inside / mean_outside if mean_outside > 0 else np.nan

    # --- Sauvegarde CSV ---
    dataset_dir = os.path.join(parent, "dataset")
    os.makedirs(dataset_dir, exist_ok=True)
    csv_path = os.path.join(dataset_dir, "results_DAPI_objet.csv")
    ...
    with open(csv_path, mode="a", newline="") as f:
        ...
        # Si nouveau fichier, écrire l'entête
        if not file_exists:
            writer.writerow([
                "condition", "num_lame", "objet", "nb_objets",
                "aire_totale", "intensite_in", "intensite_out", "ratio_in_out"
            ])
        ...
        # Ajouter la ligne
        writer.writerow([
            anat_vs_koek,        # condition : anat ou koek
            num_lame,            # numéro de la lame
            objet_type,          # type d'objet segmenté
            num_objects,         # nombre d'objets détectés
            total_area,          # aire totale
            mean_inside,         # intensité moyenne dans les objets
            mean_outside,        # intensité moyenne à l'extérieur
            ratio                # ratio intensité intérieur/extérieur
        ])

    print(f"Ligne ajoutée au CSV : {anat_vs_koek}, lame {num_lame}, {objet_type}, ratio {ratio:.3f}")
